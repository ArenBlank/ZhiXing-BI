...
export default function FileUploader({ userId, sessionId, onUploaded }) {
  ...
  return (
    <div className={`glass-panel rounded-xl p-4 transition-all duration-500 ${dragOver ? "border-blue-400 shadow-lg" : ""}`}
      ...
      {files.length > 0 && (
        <div className="mb-2 space-y-0.5 max-h-32 overflow-y-auto">
          {files.map((f) => (
            <div key={f.id} className="flex items-center justify-between px-2 py-1 bg-slate-50 rounded text-xs">
